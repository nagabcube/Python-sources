...
c.execute("SELECT cellrowcol FROM grid_proj ORDER BY 1")
cells = c.fetchall()

for i in range(len(cells)):
    for t in c.execute("SELECT * FROM tp_forecasts WHERE cellid = " + str(cells[i][0])):
        r.append(t)
...
for i in range(len(r)):
    if r[i][2] == '03' and r[i][1] < today:
        ...
        if r[i][1] >= tfour and r[i][2] == '18':
            print '%s %s %02d %8.3f' % (r[i][0], r[i][1], int(r[i][2]) + 3, (r[i+1][3]-r[i][3]) / 2)
c.close()
conn.close()
